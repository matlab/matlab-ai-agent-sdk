...
extractionTool = aisdk.LLMTool(@extractCustomerData, ...
    ApprovalRequest="never", ... %[control:dropdown:2476]{"position":[22,29]}
    Workspace = "agent");
%%
...
filteringTool = aisdk.LLMTool(@searchCustomerData, ...
    ApprovalRequest="never", ... %[control:dropdown:2ead]{"position":[22,29]}
    Workspace = "agent");
%%
...
%   data: {"defaultValue":"\"always\"","itemLabels":["always","once","never","never\","],"items":["\"always\"","\"once\"","\"never\"","never\","],"label":"ApprovalRequest","run":"Section"}
%---
