...
%
%   % Send a request
%   [text, message] = aisdk.llms.client.internal.callOllamaChatAPI(model, messages)
...
% Trying out a convention that the Endpoint contains the path.
% URL = nvp.Endpoint + "/api/chat";
% if ~startsWith(URL,"http")
%     URL = "http://" + URL;
% end

% jsonencode encodes a scalar string as "foo", not ["foo"].
% Wrap in a cell so it serializes as a JSON array.
if isscalar(nvp.StopSequences)
    nvp.StopSequences = {nvp.StopSequences};
end

parameters = buildParametersCall(model, messages, functions, nvp);

[response, streamedText] = nvp.sendRequestFcn(parameters,[], nvp.Endpoint,nvp.TimeOut,nvp.StreamFcn);

% If call errors, "choices" will not be part of response.Body.Data, instead
% we get response.Body.Data.error
if response.StatusCode=="OK"
    % Outputs the first generation
    if isempty(nvp.StreamFcn)
    ...
    % API may return content: null, normalize to scalar ""
...
elseif isstruct(nvp.ResponseFormat)
    parameters.format = aisdk.llms.client.internal.jsonSchemaFromPrototype(nvp.ResponseFormat);
elseif startsWith(string(nvp.ResponseFormat), asManyOfPattern(whitespacePattern)+"{")
    parameters.format = aisdk.llms.client.internal.VerbatimJSON(nvp.ResponseFormat);
end
