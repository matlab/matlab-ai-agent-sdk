...
%
%   % Send a request
%   [text, message] = aisdk.llms.client.internal.callOpenAIChatAPI(messages, functions, APIKey=apiKey)
...
[response, streamedText] = nvp.sendRequestFcn(parameters,nvp.APIKey, nvp.Endpoint, nvp.TimeOut, nvp.StreamFcn);
...
elseif isstruct(nvp.ResponseFormat)
    parameters.response_format = struct('type','json_schema',...
        'json_schema', struct('strict', true, 'name', 'computedFromPrototype', ...
            'schema', aisdk.llms.client.internal.jsonSchemaFromPrototype(nvp.ResponseFormat)));
elseif startsWith(string(nvp.ResponseFormat), asManyOfPattern(whitespacePattern)+"{")
    parameters.response_format = struct('type','json_schema',...
        'json_schema', struct('strict', true, 'name', 'providedInCall', ...
            'schema', aisdk.llms.client.internal.VerbatimJSON(nvp.ResponseFormat)));
end
