...
myMCPClient = mcpHTTPClient(endpoint) %[output:06225cbb]
%%
%[text] View the tools returned by the MCP client:
toolsAsStructs = myMCPClient.ServerTools %[output:1ed06a39]
jsonencode(toolsAsStructs{2}.inputSchema, 'PrettyPrint', true) %[output:69e1c51f]
%%
...
tools = aisdk.LLMTool(myMCPClient) %[output:4d43d93e]
%%
...
response = agent.run("What's 123 + 100100100?") %[output:0ac78cdb] %[output:17a3228c]
%%
%[text] ### Look at the history to see how the answer was found
agent.Messages %[output:0a396d58]
